...
//----------------------------------------------
typedef struct{
	uint16 			EXTI_InputLineNumber; 	// @ref EXTI_LINES_define
	GPIO_TypeDef 	*GPIO_Port;
	uint16			GPIO_PIN; 				// @ref GPIO_PINS_define
...
typedef struct{
	 ...
	 * myEXTI.EXTI_PIN = EXTI1PC1;
	 */

	uint8 				Trigger_Case; 	// Specifies RISING or FALLING or both trigger.
										// This parameter must be set based on @ref EXTI_TRIGGER_define

	uint8				IRQ_EN;			// Enable or Disable the EXTI IRQ (That will enable the IRQ mask in EXTI and NVIC controller)
