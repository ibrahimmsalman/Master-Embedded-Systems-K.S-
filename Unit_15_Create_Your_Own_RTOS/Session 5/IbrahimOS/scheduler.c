...
static void IbrahimOS_Update_TasksWaitingTime(void);
...
void SysTick_Handler(void){
	...
	IbrahimOS_Update_TasksWaitingTime();
...
void IbrahimOS_StartOS(void){
	...
	IbrahimOS_ActivateTask(&IDLE_TASK);
...
void IbrahimOS_Update_TasksWaitingTime(void){
	uint8 index;
		...
		if(OS_Control.OS_Tasks[index]->TaskState == Suspended){
			if(OS_Control.OS_Tasks[index]->TimeWaiting.Task_Block_State == enabled){
				...
				if(OS_Control.OS_Tasks[index]->TimeWaiting.Ticks_Count == 0){
					...
					IbrahimOS_Set_SVC(SVC_TaskWaitingTime);
				}
			}
		}
	}
